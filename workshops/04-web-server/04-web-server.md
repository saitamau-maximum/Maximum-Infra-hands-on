...
では早速build……の前に。先程あげた、Nginxにやってもらうことその２をするために、`.env`ファイルの中で、ポートを指定している部分を削除してください

例
```bash
VITE_API_BASE_URL=http://192.168.123.8:8080
↓
VITE_API_BASE_URL=http://192.168.123.8
```
...
`/etc/nginx/conf.d`というディレクトリの中に配置してください。
